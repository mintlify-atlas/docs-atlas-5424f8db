...
<Steps>
  <Step title="Token extraction">
    On the server, the `Storage` class reads tokens from the incoming HTTP request's cookie header. No `localStorage` or `sessionStorage` is accessed because those APIs do not exist in a Node.js environment.
  </Step>
...
When `ssr: true` is set in your `nuxt.config.ts`, `@nuxt-alt/auth` automatically registers a Nitro server handler at `POST /_auth/reset`. This endpoint clears the auth cookie from the server side, ensuring that a logout operation correctly removes the session cookie even when the HTTP-only flag prevents client-side JavaScript from doing so.
